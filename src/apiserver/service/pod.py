...
class AdminPodService(ServiceInterface):
    ...
    async def create(self,
        ...
        if err is None:
            await app.add_task(
                handle_pod_create_event(self.parent, PodCreateEvent(pod_id=pod.pod_id, uid=pod.uid))
            )
    ...
    async def update(self,
        ...
        if err is None:
            await app.add_task(
                handle_pod_update_event(self.parent, PodUpdateEvent(pod_id=pod.pod_id, uid=pod.uid))
            )
    ...
    async def delete(self,
        ...
        if err is None:
            await app.add_task(
                handle_pod_delete_event(self.parent, PodDeleteEvent(pod_id=pod.pod_id, uid=pod.uid))
            )
