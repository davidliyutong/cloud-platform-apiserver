...
build.docker.buildx:
	docker buildx build --platform=linux/amd64,linux/arm64 -t davidliyutong/clpl-apiserver:${GIT_VERSION} -t davidliyutong/clpl-apiserver:latest -f manifests/docker/Dockerfile .
	docker buildx build --load -t davidliyutong/clpl-apiserver:latest -f manifests/docker/Dockerfile .

push.docker.buildx:
	docker buildx build --push --platform=linux/amd64,linux/arm64 -t davidliyutong/clpl-apiserver:${GIT_VERSION} -t davidliyutong/clpl-apiserver:latest -f manifests/docker/Dockerfile .
